...
const ReleaseHealthCheckSimplified = () => {
  const navigate = useNavigate();
  
  // State for Release Notes data
  ...
  const [releaseNotesError, setReleaseNotesError] = useState(null);
  ...
  // State for K8s Pod data
  const [k8sPodData, setK8sPodData] = useState({
    totalServices: 0,
    failedServices: 0,
    notDeployedServices: 0,
    successServices: 0,
    services: []
  });
  const [k8sPodLoading, setK8sPodLoading] = useState(false);
  const [k8sPodError, setK8sPodError] = useState(null);
  ...
  React.useEffect(() => {
    const fetchReleaseNotes = async () => {
      ...
      try {
        const response = await fetch('http://localhost:5678/webhook/GetReleaseNoteList');
        if (!response.ok) {
        ...
        // Eğer data.workItems bir array ise
        else if (data.workItems && Array.isArray(data.workItems)) {
          workItemsArray = data.workItems;
        }
        // Eğer tek bir obje geliyorsa, array'e çevir
        else if (data.workItemId) {
          workItemsArray = [data];
        }
        
        // Transform API response to expected format
        const transformedWorkItems = workItemsArray.map(item => ({
          id: String(item.workItemId || item.id || ''),
          title: item.title || '',
          description: item.description || item.detail || '',
          service: item.service || item.areaPath || '',
          owner: item.responsible || item.owner || '',
          jiraId: item.jiraId || null,
          azureStatus: item.status || item.azureStatus || '',
          jiraStatus: item.jiraStatus || null,
          version: item.version || '',
          jiraVersion: item.jiraVersion || null,
          impactScope: item.impactScope || '',
          dontNeedReleaseNote: item.dontNeedReleaseNote || false,
          reviewStatus: item.reviewStatus || 'Pending',
          aiStatus: item.aiStatus && item.aiStatus !== '-' ? item.aiStatus : null,
          issueType: item.issueType || (item.status === 'Ok' ? '' : 'status-issue'),
          workItemLink: item.detail || item.workItemLink || ''
        }));
        
        // Calculate issues count (items with non-empty issueType)
        const issuesCount = transformedWorkItems.filter(item => item.issueType && item.issueType !== '').length;
        
        setReleaseNotesData({
          totalWorkItems: transformedWorkItems.length,
          issuesCount: issuesCount,
          workItems: transformedWorkItems
        });
      } catch (error) {
        console.error('Error fetching release notes:', error);
        setReleaseNotesError(error.message);
    ...
    fetchReleaseNotes();
  }, [selectedVersion]); // Re-fetch when version changes
  ...
  React.useEffect(() => {
    const fetchPipelineData = async () => {
      ...
      try {
        const response = await fetch('http://localhost:5678/webhook/GetPipelineBuilds');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        console.log('Pipeline API Response:', data); // Debug için
        console.log('Total builds received:', data.length); // Kaç kayıt geldiğini göster
        
        // API'den gelen array'i transform et - TÜM KAYITLARI AL
        const buildsArray = Array.isArray(data) ? data : [];
        
        // Transform to expected format - HER BUILD'İ AYRI GÖR
        const transformedServices = buildsArray.map(build => ({
          name: build.serviceName || '',
          owner: build.responsible || '',
          status: build.state === 'completed' 
            ? (build.result === 'succeeded' ? 'Completed' : build.result === 'failed' ? 'Failed' : 'Completed')
            : build.state === 'inProgress' ? 'InProgress' : build.state,
          pipelineUrl: build.link || '',
          buildId: build.id,
          buildName: build.name,
          createdDate: build.createdDate,
          runFortify: build.runFortify,
          result: build.result,
          state: build.state,
          aiStatus: null, // API'den gelirse kullan, yoksa null
          aiMessage: null,
          prUrl: null
        }));
        ...
        const completedServices = transformedServices.filter(s => 
          s.status === 'Completed' && s.result === 'succeeded'
        ).length;
        const failedServices = transformedServices.filter(s => 
          s.status === 'Failed' || (s.status === 'Completed' && s.result === 'failed')
        ).length;
        const inProgressServices = transformedServices.filter(s => 
          s.status === 'InProgress' || s.status === 'inProgress'
        ).length;
  ...
  // Fetch K8s Pod data
  React.useEffect(() => {
    const fetchK8sPodData = async () => {
      setK8sPodLoading(true);
      setK8sPodError(null);
      
      try {
        const response = await fetch('http://localhost:5678/webhook/GetPods');
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
        console.log('K8s Pod API Response:', data);
        console.log('Total pods received:', data.length);
        
        // API'den gelen array'i transform et
        const podsArray = Array.isArray(data) ? data : [];
        
        // Transform to expected format
        const transformedPods = podsArray.map(pod => ({
          name: pod.serviceName || pod.name || '',
          owner: pod.responsible || '',
          podName: pod.podName || '',
          namespace: pod.namespace || '',
          imageName: pod.imageName || '',
          currentVersion: pod.currentVersion || '',
          latestVersion: pod.latestVersion || '',
          targetVersion: pod.latestVersion || '', // latestVersion'ı target olarak kullan
          podStatus: pod.status || '',
          status: pod.status === 'Running' ? 'Success' : 
                  pod.status === 'CrashLoopBackOff' || pod.status === 'Error' ? 'Failed' : 
                  pod.status === 'NotDeployed' ? 'NotDeployed' : pod.status,
          logs: null // Şimdilik logs yok, gerekirse eklenebilir
        }));
        
        // Calculate statistics
        const totalServices = transformedPods.length;
        const successServices = transformedPods.filter(p => p.status === 'Success').length;
        const failedServices = transformedPods.filter(p => p.status === 'Failed').length;
        const notDeployedServices = transformedPods.filter(p => p.status === 'NotDeployed').length;
        ...
        setK8sPodData({
          ...
          notDeployedServices,
          successServices,
          services: transformedPods
        });
      } catch (error) {
        console.error('Error fetching K8s pod data:', error);
        setK8sPodError(error.message);
        setK8sPodData({
          totalServices: 0,
          failedServices: 0,
          notDeployedServices: 0,
  ...
  // Mock pipeline data - REMOVED, now using API data above
  
  // Mock K8s Pod Status data - REMOVED, now using API data above
  
  const releaseNotesSuccessRate = releaseNotesData.totalWorkItems > 0 
    ? (((releaseNotesData.totalWorkItems - releaseNotesData.issuesCount) / releaseNotesData.totalWorkItems) * 100).toFixed(0)
    : 0;
...
[FATAL] Email service initialization failed. Container restarting...`
  ...
  const successRate = pipelineData.totalServices > 0 
    ? ((pipelineData.completedServices / pipelineData.totalServices) * 100).toFixed(0)
    : 0;
  ...
  const k8sSuccessRate = k8sPodData.totalServices > 0
    ? ((k8sPodData.successServices / k8sPodData.totalServices) * 100).toFixed(0)
    : 0;
  ...
  const ownerData = pipelineData.services.length > 0 
    ? pipelineData.services.reduce((acc, service) => {
  ...
  const k8sOwnerData = k8sPodData.services.length > 0
    ? k8sPodData.services
          ...
          <AccordionDetails sx={{ p: 3 }}>
            {pipelineLoading ? (
              <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', py: 5 }}>
                <Typography variant="body2" color="text.secondary">
                  Pipeline verileri yükleniyor...
                </Typography>
              </Box>
            ) : pipelineError ? (
              <Alert severity="error" sx={{ mb: 3 }}>
                Pipeline verileri yüklenirken hata oluştu: {pipelineError}
              </Alert>
            ) : pipelineData.services.length === 0 ? (
              <Alert severity="info" sx={{ mb: 3 }}>
                Henüz hiç pipeline build'i bulunamadı.
              </Alert>
            ) : (
              <>
                {/* Description */}
                <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                  ...
                  Toplam {pipelineData.services.length} build kaydı gösteriliyor.
                </Typography>
              ...
              <Grid item xs={12} md={4}>
                <Paper variant="outlined" sx={{ p: 3, height: '100%' }}>
                  <Typography variant="subtitle1" fontWeight="bold" gutterBottom>
                    Sorumlulara Göre Build Dağılımı
                  </Typography>
                    ...
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {pipelineData.services.map((service, index) => {
                          ...
                          const createdDate = service.createdDate 
                            ...
                            <TableRow key={index} hover>
                              ...
                              <TableCell>
                                <Chip 
                                  label={service.state} 
                                  size="small" 
                                  color={service.state === 'completed' ? 'default' : 'warning'}
                                  variant="outlined"
                                />
                              </TableCell>
                              <TableCell>
                                <Chip 
                                  ...
                                  size="small" 
                                  color={
                                    service.result === 'succeeded' ? 'success' : 
                                    service.result === 'failed' ? 'error' : 
                                    'default'
                                  }
                                  variant="outlined"
                                />
                              ...
                              <TableCell>
                                <Chip 
                                  label={service.runFortify ? 'Evet' : 'Hayır'} 
                                  size="small" 
                                  color={service.runFortify ? 'primary' : 'default'}
                                  variant="outlined"
                                />
                              </TableCell>
                              <TableCell>
        ...
        <Paper elevation={3} sx={{ mt: 3, borderRadius: 2, overflow: 'hidden' }}>
          <Accordion>
            ...
            <AccordionDetails sx={{ p: 3 }}>
              ...
              ) : k8sPodData.services && k8sPodData.services.length === 0 ? (
                <Alert severity="info" sx={{ mb: 2 }}>
                  Henüz pod verisi bulunmuyor.
                </Alert>
              ) : (
                <>
                  <Grid container spacing={3}>
                          ...
                          <Table size="small" stickyHeader>
                            <TableHead>
                              <TableRow>
                                ...
                                <TableCell><strong>Mevcut Versiyon</strong></TableCell>
                                <TableCell><strong>Son Versiyon</strong></TableCell>
                                <TableCell><strong>Durum</strong></TableCell>
                                <TableCell align="center"><strong>Aksiyonlar</strong></TableCell>
                              </TableRow>
                            </TableHead>
                            <TableBody>
                              {k8sPodData.services.map((service, index) => {
                                const isOutdated = service.currentVersion !== service.latestVersion;
                                return (
                                  <TableRow 
                                    key={index}
                                    sx={{ 
                                      bgcolor: isOutdated ? 'warning.lighter' : 'inherit',
                                      '&:hover': { bgcolor: isOutdated ? 'warning.light' : 'action.hover' }
                                    }}
                                  >
                                    <TableCell>
                                    ...
                                    <TableCell>
                                      <Chip 
                                        label={service.podStatus} 
                                        size="small" 
                                        color={getPodStatusColor(service.podStatus)}
                                        variant="outlined"
          ...
          <Accordion>
            ...
            <AccordionDetails sx={{ p: 3 }}>
              {releaseNotesLoading ? (
                <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', py: 5 }}>
                  <Typography variant="body2" color="text.secondary">
                    Release Notes verileri yükleniyor...
                  </Typography>
                </Box>
              ) : releaseNotesError ? (
                <Alert severity="error" sx={{ mb: 3 }}>
                  Release Notes verileri yüklenirken hata oluştu: {releaseNotesError}
                </Alert>
              ) : releaseNotesData.workItems.length === 0 ? (
                <Alert severity="info" sx={{ mb: 3 }}>
                  Henüz hiç work item bulunamadı.
                </Alert>
              ) : (
                <>
                  {/* Description */}
...
export default ReleaseHealthCheckSimplified;
