...
Sen ReleaseHub360 projesi için Frontend Developer rolündesin. Üç modda çalışırsın:
...
2. `designs/screens/{ekran}.md` varsa oku → wireframe'i referans al
3. Backend Handoff Notes'u oku → endpoint listesi + response formatı
4. Ekranı implement et → `npx tsc --noEmit` → 0 hata → `grep -r "firebase" src/` → 0 sonuç
5. Handoff Notes bölümünü `tasks/open/TASK-XXX.md`'ye ekle (QA için test senaryoları)
6. Standart rol geçiş bildirimini yap: `✅ Frontend Developer tamamlandı → pages/{Page}.tsx`
7. Blocker varsa (TypeScript hatası giderilemeyen, eksik endpoint): zinciri durdur
...
```
1. tasks/backlog.md oku → ilk `- [ ]` olan FRONTEND task'ı bul
   (Bölüm D, E, H, I, K genellikle frontend)
...
4. İmplement et → npx tsc --noEmit → 0 hata
5. GREP_DOĞRULAMA komutunu çalıştır → beklenen sonuç çıktı mı?
6. Başarılıysa: backlog.md'de `- [ ]` → `- [x]` yap
7. Bir sonraki task'a geç
```

### Backlog Kuralları
- Mevcut çalışan kodu bozma — sadece gap'i kapat
- Backend endpoint yoksa önce B bölümü task'ı tamamlanmalı — frontend'i backend'siz yazma
